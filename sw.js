 ...
 * pouvoir bloquer un visiteur (symptôme observé : un ancien SW resté en cache
 * servait une version cassée en navigation normale, alors que la navigation
 * privée — sans SW — fonctionnait). Les garde-fous :
 *
 ...
 *     jamais dans le precache — une police manquante ne doit pas faire échouer
 *     l'installation.
 *  2. PRECACHE TOLÉRANT : chaque ressource est mise en cache indépendamment
 *     (jamais de `cache.addAll` strict qui rejette tout si un seul fichier
 *     échoue). Un échec unitaire n'interrompt pas l'installation.
 ...
 *     pour rester valable quel que soit le sous-chemin de déploiement.
 *
...
const VERSION_CACHE = 'mayeutik-v5';
 ...
 * et mise en cache séparément ; un échec unitaire est ignoré et n'interrompt
 * pas l'installation.
 */
async function precacherTolerant() {
  ...
  await Promise.all(fichiers.map(async (url) => {
    try {
      ...
      if (reponse && reponse.ok) await cache.put(url, reponse.clone());
    } catch (e) {
...
/* Activation : purge des caches obsolètes + prise de contrôle immédiate des
   pages déjà ouvertes (garde-fou 4). */
 ...
 * par une autre page). Le cache ne sert qu'en repli hors ligne ; en dernier
 * recours on relaie l'erreur réseau native plutôt qu'une réponse vide.
 */
async function reseauPuisCache(requete) {
  try {
    const reponse = await fetch(requete);
    if (reponse && reponse.ok) mettreEnCache(VERSION_CACHE, requete, reponse.clone());
    return reponse;
  } catch (e) {
 ...
 * (garde-fou 3). Le repli de navigation vers index.html ne concerne QUE la
 * coquille (les jeux ne passent jamais par ici).
 */
